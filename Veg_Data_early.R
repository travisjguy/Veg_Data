...
length_veg_early <- sum(veg$Length[veg$Season == "early"]) ## trying to find the length of all early season combined

length_veg_early   #should equal 179,974 and it does

total_length <- veg %>% group_by(Species_clean) %>%
  summarize(total_length = sum(Length[Season = "early"]),
            percent_length = sum(Length[Season = "early"])/length_veg_early)

length_veg_late <- sum(veg$Length[veg$Season == "late"])  ## finding length of all late season sum

length_veg_late    #should equal 180,000 and it does

total_length <- veg %>% group_by(Species_clean) %>%
  summarize(total_length = sum(Length[Season = "late"]),
            percent_length = sum(Length[Season = "late"])/length_veg_late)
